...
simPower_b <- function(d, inc, n.eq = TRUE, csv = TRUE){
  n1 = vector()
  n2 = vector()
  n_tot = vector()
  t_stat = vector()
  ...
  p=seq(0.0,0.99,inc)
  i=1
if(n.eq == TRUE){for(i in i:length(p)){
  ...
  plot(p, BF10_v.wide)
  abline(v=.80, col="dodgerblue2", lty = 2, lwd = 3)
  spearman.ci(p, BF10_v.wide, 1000, .95)
} else if(n.eq == FALSE){for(i in i:length(p)){
...
simPower_w <- function(d, inc, csv = TRUE){
  n1 = vector()
  n2 = vector()
  n_tot = vector()
  t_stat = vector()
